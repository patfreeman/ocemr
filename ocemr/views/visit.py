...
def get_visit_menu(current, patient):
    ...
    if patient.gender == "F":
        menu.append(
            {"link": "preg", "ord": 9, "title": "Pregnancy History", "active": False}
        )
    ...
    if old_v != None:
        ...
        for old_diag in old_diags:
            ...
            if d == None:  # Create the record.
                d, is_new = Diagnosis.objects.get_or_create(
                    type=old_diag.type, patient=p, visit=v, diagnosedBy=request.user
                )
                if is_new:
                    d.save()
...
def visit_subj_new(request, id, symptomtypeid):
    ...
    if request.method == "POST":  # If the form has been submitted...
        form = NewVisitSymptomForm(
            vid, stid, request.POST
...
def visit_obje_examNote_new(request, id, examnotetypeid):
    ...
    if request.method == "POST":  # If the form has been submitted...
        form = NewExamNoteForm(
            v, ent, request.user, request.POST
        )  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
            o = form.save()
            return HttpResponseRedirect("/close_window/")
...
def visit_cost_estimate_detail(request, id):
    ...
    for row in ced:
        table += "<TR><TD>%s<TD>%s<TD>%s<TD>%s</TR>" % (row[0], row[1], row[2], row[3])
    table += "<TR><TD COLSPAN=3>TOTAL<TD>%s</TR>" % (v.get_estimated_visit_cost())
    return render(
        request,
        "popup_table.html",
        {
...
def visit_record(request, id, type):
    ...
    next_visits = Visit.objects.filter(
        patient=v.patient, scheduledDate__gt=v.scheduledDate
    )
    upco_text = ""
    ...
    if type == "print":
        ...
        p = Popen(cmd, shell=True, bufsize=0, stdin=PIPE, stdout=PIPE, close_fds=True)
        ...
        return render(
            ...
            {
                "lines": lines,
                "link_text": """<a href="#" onclick="window.print();return false;">Print</a>""",
            },
        )
