...
def patient_edit_age(request, id):
    ...
    if request.method == "POST":  # If the form has been submitted...
        ...
        form = EditPatientAgeForm(
            initial={"birthYear": p.birthYear, "birthDate": p.birthDate}
        )  # An unbound form
    return render(
        request,
        "popup_form.html",
...
def patient_edit_alt_contact_name(request, id):
    ...
    p = Patient.objects.get(pk=id)
    if request.method == "POST":  # If the form has been submitted...
        form = EditPatientAltContactNameForm(
            request.POST
        )  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
        ...
        form = EditPatientAltContactNameForm(
            initial={"alt_contact_name": p.altContactName}
        )  # An unbound form
    return render(
        request,
        "popup_form.html",
...
def patient_edit_alt_contact_phone(request, id):
    ...
    p = Patient.objects.get(pk=id)
    if request.method == "POST":  # If the form has been submitted...
        form = EditPatientAltContactPhoneForm(
            request.POST
        )  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
        ...
        form = EditPatientAltContactPhoneForm(
            initial={"alt_contact_phone": p.altContactPhone}
        )  # An unbound form
    return render(
        request,
        "popup_form.html",
...
def patient(request, id):
    ...
    p = Patient.objects.get(pk=id)
    upcoming_visits = (
        Visit.objects.filter(patient=p).filter(status="SCHE").order_by("scheduledDate")
    )
    visits = (
        Visit.objects.filter(patient=p).exclude(status="SCHE").order_by("-seenDateTime")
    )
    return render(
        request,
        "patient.html",
        ...
        if form.is_valid():  # All validation rules pass
            ...
            if name != "":
                ...
                for term in name.split():
                    if i:
                        q_name = q_name & (
                            Q(familyName__icontains=term) | Q(givenName__icontains=term)
                        )
                    else:
                        q_name = Q(familyName__icontains=term) | Q(
                            givenName__icontains=term
                        )
                    i += 1
...
def schedule_new_visit(request, id):
    ...
    if request.method == "POST":  # If the form has been submitted...
        form = NewScheduledVisitForm(
            request.user, p, request.POST
        )  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
            o = form.save()
            return HttpResponseRedirect("/close_window/")
...
def edit_visit(request, id):
    ...
    if request.method == "POST":  # If the form has been submitted...
        form = EditScheduledVisitForm(
            v, request.user, request.POST
        )  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
...
def edit_visit_seen(request, id):
    ...
    if request.method == "POST":  # If the form has been submitted...
        ...
        if form.is_valid():  # All validation rules pass
            v.seenDateTime = "%s %s" % (
                form.cleaned_data["seenDate"],
                form.cleaned_data["seenTime"],
            )
            v.save()
            return HttpResponseRedirect("/close_window/")
    else:
        form = EditVisitSeenForm(
            initial={
                "seenDate": formats.date_format(v.seenDateTime, "SHORT_DATE_FORMAT"),
                "seenTime": formats.date_format(v.seenDateTime, "TIME_FORMAT"),
            }
        )
    return render(
        request,
