...
class EditPatientNoteForm(forms.Form):
    ...
    patient = forms.ModelChoiceField(
        queryset=Patient.objects.all(), widget=forms.HiddenInput
    )
...
class NewWalkinVisitForm(forms.ModelForm):
    ...
    def __init__(self, user, p, *args, **kwargs):
        ...
        self.fields["seenDateTime"].initial = datetime.now().strftime(
            "%Y-%m-%d %H:%M:%S"
        )
...
class NewPatientForm(forms.ModelForm):
    ...
    createdBy = forms.ModelChoiceField(
        queryset=User.objects.all(), widget=forms.HiddenInput
    )
...
class NewVisitSymptomForm(forms.ModelForm):
    ...
    type = forms.ModelChoiceField(
        queryset=SymptomType.objects.all(), widget=forms.HiddenInput
    )
    visit = forms.ModelChoiceField(
        queryset=Visit.objects.all(), widget=forms.HiddenInput
    )
...
class NewVitalsForm(forms.Form):
    ...
    observedBy = forms.ModelChoiceField(
        queryset=User.objects.all(), widget=forms.HiddenInput
    )
    ...
    bloodPressureSystolic = forms.IntegerField(
        label="BP-Systolic", help_text="in mmHg.", required=False
    )
    bloodPressureDiastolic = forms.IntegerField(
        label="BP-Diastolic", help_text="in mmHg.", required=False
    )
    heartRate = forms.IntegerField(
        label="Heart Rate", help_text="in beats per minute.", required=False
    )
    respiratoryRate = forms.IntegerField(
        label="Respiratory Rate", help_text="in breaths per minute.", required=False
    )
...
class NewPregnancyForm(forms.Form):
    ...
    addedBy = forms.ModelChoiceField(
        queryset=User.objects.all(), widget=forms.HiddenInput
    )
    ...
    deliveryMode = forms.ChoiceField(
        label="Delivery Mode", choices=Pregnancy.PREGNANCY_DELIVERY_MODES, required=True
    )
    gender = forms.ChoiceField(
        label="Gender", required=True, choices=Pregnancy.GENDER_CHOICES
    )
    presentation = forms.CharField(
        label="Presentation", help_text="Vertex / Breech / other", required=False
    )
    laborLength = forms.CharField(
        label="Labor Length", help_text="Time 'HH:MM'", required=True
    )
    ...
    breastfeedingProblems = forms.NullBooleanField(
        label="Breastfeeding Problems", required=False, widget=forms.NullBooleanSelect
    )
...
class NewExamNoteForm(forms.ModelForm):
    ...
    type = forms.ModelChoiceField(
        queryset=ExamNoteType.objects.all(), widget=forms.HiddenInput
    )
    visit = forms.ModelChoiceField(
        queryset=Visit.objects.all(), widget=forms.HiddenInput
    )
...
class NewDiagnosisForm(forms.ModelForm):
    ...
    type = forms.CharField(
        widget=widgets.JQueryAutoContains("/autosearch_title/ocemr/DiagnosisType/")
    )
    patient = forms.ModelChoiceField(
        queryset=Patient.objects.all(), widget=forms.HiddenInput
    )
    ...
    diagnosedBy = forms.ModelChoiceField(
        queryset=User.objects.all(), widget=forms.HiddenInput
    )
    status = forms.CharField(widget=forms.HiddenInput)
...
class NewMedForm(forms.ModelForm):
    ...
    type = forms.CharField(
        widget=widgets.JQueryAutoContains("/autosearch_title/ocemr/MedType/")
    )
    diagnosis = forms.ModelChoiceField(
        queryset=Diagnosis.objects.all(), widget=forms.HiddenInput
    )
    patient = forms.ModelChoiceField(
        queryset=Patient.objects.all(), widget=forms.HiddenInput
    )
...
class ChangePasswordForm(forms.Form):
    ...
    newPasswordAgain = forms.CharField(
        widget=forms.PasswordInput, label="New Password Again"
    )
