...
def post_syncdb_auto_upgrade(sender, **kwargs):
    ...
    if DATABASE_ENGINE not in ("mysql", "sqlite3"):
        print "Skipping post_syncdb_auto_upgrade because %s is an unsupported DATABASE_ENGINE." % (
            DATABASE_ENGINE
        )
        return
    ...
    for fname in sorted(os.listdir("%s/schema_updates" % CONTRIB_PATH)):
        ...
        if m:
            ...
            if DEBUG:
                print "found %s: major=%d, minor=%d" % (fname, f_major, f_minor)
            if f_major > latest_version.major or (
                f_major == latest_version.major and f_minor > latest_version.minor
            ):
                print "applying %s..." % (fname)
                if f_major > biggest_major:
                    biggest_major = f_major
                if f_minor > biggest_minor:
                    biggest_minor = f_minor
        ...
        if m:
            ...
            if f_major > latest_version.major or (
                ...
                os.system(cmd)
    if biggest_major > latest_version.major or (
        biggest_major == latest_version.major and biggest_minor > latest_version.minor
    ):
