...
class PSDoc:
    ...
    def __init__(self, font_size=12, margins=None, debug=False):
        ...
        if self.debug:
            # Left
            self.ps.line(
                (self.dpi * self.margin_left, self.dpi * self.margin_bottom),
                (
    ...
    def text(self, text, font_size=None, indent=0):
        ...
        indent = float(indent)
        if self.cur_h == -1:
            self.cur_h = int(self.dpi * (self.page_height - self.margin_top)) - (
                font_size
            )
        ...
        while text != "":
            ...
            max_chars = int(
                (
                    ...
                    * (
                        self.page_width
                        - (self.margin_left + self.margin_right + indent)
                    )
                )
            ...
            self.ps.text(
                (int(self.dpi * (self.margin_left + indent + wrapped)), self.cur_h),
                line.encode("utf-8"),
            )
            text = text[max_chars:]
